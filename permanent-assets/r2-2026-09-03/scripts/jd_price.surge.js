...
*/
// Self-hosted Surge adaptation snapshot from wf021325/qx main
const $ = new Env("京东比价");
...
if (url.includes('manmanbuy.com')) {
    const captured = captureManmanbuyCK();
    if (captured) {
        $.msg($.name, '获取CK成功🎉', '慢慢买参数已保存');
    }
    $.done({});
}

function captureManmanbuyCK() {
    const candidates = [];
    const push = v => {
        if (typeof v === 'string' && v) {
            candidates.push(v);
            try { candidates.push(decodeURIComponent(v)); } catch (_) {}
        }
    };
    ...
    const collected = {};
    const absorbPairs = str => {
        if (!str || typeof str !== 'string') return;
        const q = str.includes('?') ? str.split('?').slice(1).join('?') : str;
        ...
        try {
            ...
            const walk = o => {
                ...
                for (const [k, v] of Object.entries(o)) {
                    ...
                    else if (typeof v === 'string' || typeof v === 'number' || typeof v === 'boolean') collected[k] = String(v);
                }
            };
            walk(obj);
        } catch (_) {}
    ...
    candidates.forEach(absorbPairs);

    // Prefer a complete original form body if it already contains c_mmbDevId.
    const raw = candidates.find(x => /(?:^|[?&])c_mmbDevId=/.test(x));
    let value = raw && !raw.includes('?') ? raw : '';

    if (!value && collected.c_mmbDevId) {
        const preferred = Object.fromEntries(
            Object.entries(collected).filter(([k]) =>
                k === 'c_mmbDevId' || k.startsWith('c_') || ['deviceId','deviceid','idfa','idfv'].includes(k)
            )
        );
        value = Object.entries(preferred)
            .map(([k, v]) => `${encodeURIComponent(k)}=${encodeURIComponent(v)}`)
            .join('&');
    }

    if (!value || !/(?:^|&)c_mmbDevId=/.test(value)) return false;

    const old = $.getdata(manmanbuy_key);
    const ok = $.setdata(value, manmanbuy_key);
    return !!ok && value !== old;
}
